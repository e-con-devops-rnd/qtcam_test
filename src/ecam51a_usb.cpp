...
bool ecam51A_USB::selectFocusMode(uint mode)
{
    ...
    {
        qDebug()<<"Failed setting Focus Mode";
        return false;
    }
    return true;
}
...
bool ecam51A_USB::getFocusPosition()
{
    ...
    if(reg_value == 0)
    {
         if(read_sensor_reg(SENSOR_MANUAL_FOCUS_REG_ADDR_5M, &reg_value) == SUCCESS)
         {
               ...
               qDebug()<<"FocusPosition : "<<FocusPosition;
...
bool ecam51A_USB::setManualFocusPosition(uint FocusPosition)
{
    ...
    {
        qDebug()<<"SetManualFocus Failed,FocusPosition: "<<FocusPosition;
        return false;
    }
}
