...
int Videostreaming::jpegDecode(unsigned char **pic, unsigned char *buf, unsigned long bytes_used)
{
    ...
    if((srcbuf = (unsigned char *)malloc(srcsize))==NULL)
        printf("allocating srcbuf memory failed");

    memcpy(srcbuf, buf, srcsize);

    if((handle = tjInitTransform()) == NULL)
        printf("executing tjInitTransform()");

    if(tjDecompressHeader3(handle, srcbuf, srcsize, &w, &h, &subsamp, &cs)==-1)
        printf("tjDecompressHeader3():%s",tjGetErrorStr());
    ...
    for(tilew=dotile? 16:w, tileh=dotile? 16:h; ; tilew*=2, tileh*=2)
    {
        ...
        if((jpegbuf = (unsigned char **)malloc(sizeof(unsigned char *)
            ...
            printf("allocating JPEG tile array ");

        memset(jpegbuf, 0, sizeof(unsigned char *)*ntilesw*ntilesh);

        if((jpegsize = (unsigned long *)malloc(sizeof(unsigned long)
                                             *ntilesw*ntilesh)) == NULL)
            printf("allocating JPEG size array");
